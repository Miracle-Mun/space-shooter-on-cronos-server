...
export class CrosmoRoom extends Room<CrosmoState> {
  ...
  async onCreate(options: IRoomData) {
    ...
    this.timer = setTimeout(()=>{this.clients.forEach((client:Client) => {
      client.send(
      Message.SEND_GAME_TIMEOUT, {  
    ...
    this.onMessage(
      ...
      (client, message: {
        ...
        this.clients.forEach((clientData, id) => {
          if(clientData.sessionId != client.sessionId) {
            clientData.send(Message.GET_PLAYERS, {
      ...
      if(bullet.distanceTravelled>1600){
        ...
        this.state.players?.forEach((player, id) => {
          if (bullet.owner != id) {
              ...
              let dist = Math.sqrt(dx * dx + dy * dy);
              if (dist < bulletRange && bullet.teamflag != player.team && player.isExplode ===false  ) {
                this.broadcast(
                  Message.COLLIDE_PLAYER_BULLET, {
                  punished_id: id,
